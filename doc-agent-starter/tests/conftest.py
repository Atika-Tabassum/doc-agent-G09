...
# to clear preprocess.py's blank-page threshold (measured ink ratio ~0.0016 vs. the 0.002 cutoff) —
# every synthetic page below silently gets classified as blank and dropped, which is exactly what
# caused every downstream ingest/OCR test that depends on this fixture to fail with "preprocessed 0
# pages". Never use that zero-arg fallback here.
...
def _load_test_font(size: int):
    ...
    zero-arg `load_default()` (tiny, ~6x11px, effectively unreadable) is intentionally never
    used."""
...
def synthetic_corpus(tmp_path):
    ...
    metadata sidecars), not Bengali OCR quality, which belongs in notebooks/kb_demo.ipynb against
    the real corpus."""
    import numpy as np
    ...
    for pnum, lines in enumerate(pages, start=1):
        ...
        assert ink_ratio > _BLANK_INK_PIXEL_RATIO * 5, (
            ...
            f"preprocess.py's blank-page threshold ({_BLANK_INK_PIXEL_RATIO}) in this "
            f"environment — the fixture's synthetic rendering isn't producing legible pages here."
        )
